...
class Company extends Admin_Controller 
{
    ...
    */
	...
	{  
		...
		$this->form_validation->set_rules('company_name', 'Tên công cty', 'trim|required');
		$this->form_validation->set_rules('service_charge_value', 'Số tiền phí', 'trim|integer');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Khởi tạo thành công');
        		redirect('company/', 'refresh');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Xuất hiện lỗi!!');
